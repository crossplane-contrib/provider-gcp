...
const (
	mockName          = "mockName"
	mockGKEClusterURI = "projects/mockProj/locations/mockLoc/clusters/mockName"
)
...
func TestGKEClusterURIReferencerGetStatus(t *testing.T) {

	errResourceNotFound := &kerrors.StatusError{ErrStatus: metav1.Status{Reason: metav1.StatusReasonNotFound}}

	readyResource := GKECluster{
		Status: GKEClusterStatus{
			AtProvider: GKEClusterObservation{
				SelfLink: URIPrefix + mockGKEClusterURI,
			},
		},
	}
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			r := GKEClusterURIReferencer{LocalObjectReference: corev1.LocalObjectReference{Name: mockName}}
...
func TestGKEClusterURIReferencerBuild(t *testing.T) {
	type input struct {
		readerFn func(ctx context.Context, key client.ObjectKey, obj runtime.Object) error
	}
	type expected struct {
	...
	for name, tc := range map[string]struct {
		...
		"ReferenceRetrieved_ReturnsExpected": {
			input: input{
				readerFn: func(ctx context.Context, key client.ObjectKey, obj runtime.Object) error {
					p := obj.(*GKECluster)
					p.Status.AtProvider.SelfLink = URIPrefix + mockGKEClusterURI
					return nil
				},
			},
			expected: expected{
				value: mockGKEClusterURI,
			},
		},
	} {
		t.Run(name, func(t *testing.T) {
