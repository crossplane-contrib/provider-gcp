...
func SetupGKEClusterSecret(mgr ctrl.Manager, l logging.Logger) error {
	name := secret.ControllerName(v1beta1.GKEClusterGroupKind)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		WithEventFilter(resource.NewPredicates(resource.AnyOf(
			...
			resource.AllOf(resource.IsControlledByKind(v1beta1.GKEClusterGroupVersionKind), resource.IsPropagator())))).
