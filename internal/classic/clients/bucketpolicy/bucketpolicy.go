...
func GenerateBucketPolicyInstance(in v1alpha12.BucketPolicyParameters, sp *storage.Policy) {
...
// set of parameters.
func IsUpToDate(in *v1alpha12.BucketPolicyParameters, observed *storage.Policy) (bool, error) {
...
func BindRoleToMember(in v1alpha12.BucketPolicyMemberParameters, sp *storage.Policy) bool {
	sp.Version = iamv1alpha1.PolicyVersion
...
func UnbindRoleFromMember(in v1alpha12.BucketPolicyMemberParameters, sp *storage.Policy) bool {
	for _, b := range sp.Bindings {
		if b.Role == in.Role {
			ix := -1
