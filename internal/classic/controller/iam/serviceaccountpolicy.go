...
func SetupServiceAccountPolicy(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(v1alpha12.ServiceAccountPolicyGroupKind)
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(v1alpha12.ServiceAccountPolicyGroupVersionKind),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha12.ServiceAccountPolicy{}).
...
func (e *serviceAccountPolicyExternal) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	cr, ok := mg.(*v1alpha12.ServiceAccountPolicy)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errNotServiceAccountPolicy)
	}

	instance, err := e.serviceaccountspolicy.GetIamPolicy(gcp.StringValue(cr.Spec.ForProvider.ServiceAccount)).OptionsRequestedPolicyVersion(v1alpha12.PolicyVersion).Context(ctx).Do()
	if err != nil {
