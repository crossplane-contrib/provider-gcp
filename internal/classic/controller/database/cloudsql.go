...
import (
	...
	"github.com/crossplane/provider-gcp/internal/classic/clients/cloudsql"
...
func SetupCloudSQLInstance(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(v1beta12.CloudSQLInstanceGroupKind)
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(v1beta12.CloudSQLInstanceGroupVersionKind),
...
func (c *cloudsqlExternal) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	cr, ok := mg.(*v1beta12.CloudSQLInstance)
	if !ok {
	...
	case v1beta12.StateRunnable:
		cr.Status.SetConditions(xpv1.Available())
	case v1beta12.StateCreating:
		cr.Status.SetConditions(xpv1.Creating())
	case v1beta12.StateCreationFailed, v1beta12.StateSuspended, v1beta12.StateMaintenance, v1beta12.StateUnknownState:
...
func (c *cloudsqlExternal) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	if cr.Status.AtProvider.State == v1beta12.StateCreating {
...
func getConnectionDetails(cr *v1beta12.CloudSQLInstance, instance *sqladmin.DatabaseInstance) managed.ConnectionDetails {
	m := managed.ConnectionDetails{
		xpv1.ResourceCredentialsSecretUserKey: []byte(cloudsql.DatabaseUserName(cr.Spec.ForProvider)),
		v1beta12.CloudSQLSecretConnectionName: []byte(instance.ConnectionName),
	}
	...
	for _, ip := range cr.Status.AtProvider.IPAddresses {
		if ip.Type == v1beta12.PrivateIPType {
			m[v1beta12.PrivateIPKey] = []byte(ip.IPAddress)
		...
		if ip.Type == v1beta12.PublicIPType {
			m[v1beta12.PublicIPKey] = []byte(ip.IPAddress)
