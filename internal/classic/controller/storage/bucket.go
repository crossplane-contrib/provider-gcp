...
func SetupBucket(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(v1alpha32.BucketGroupKind)
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(v1alpha32.BucketGroupVersionKind),
...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	if err := mergo.Merge(proposed, v1alpha32.NewBucketSpecAttrs(a)); err != nil {
	...
	cr.Status.BucketOutputAttrs = v1alpha32.NewBucketOutputAttrs(a)
	cr.SetConditions(xpv1.Available())

	return managed.ExternalObservation{
		ResourceExists:   true,
		ResourceUpToDate: cmp.Equal(v1alpha32.NewBucketUpdatableAttrs(a), &cr.Spec.BucketUpdatableAttrs),
	}, nil
}
...
func (e *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	err := e.handle.Bucket(meta.GetExternalName(cr)).Create(ctx, e.projectID, v1alpha32.CopyBucketSpecAttrs(&cr.Spec.BucketSpecAttrs))
...
func (e *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	ua := v1alpha32.CopyToBucketUpdateAttrs(cr.Spec.BucketUpdatableAttrs, current.Labels)
